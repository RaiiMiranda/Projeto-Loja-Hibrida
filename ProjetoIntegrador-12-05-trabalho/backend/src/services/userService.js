...
export const userService = {
    ...
    async createUser(data) {
        ...
        const { name, email, confirmEmail, password, confirmPassword, cpf, phone } = data;

        // Se não preencher os campos obrigatórios, então não permite o cadastro
        if (!name || !email || !confirmEmail || !password || !confirmPassword || !cpf || !phone) {
            throw new Error("Dados obrigatórios não informados.");
        }

        // Verificando confirmação de email e senha
        ...
        // Se email do usuário já existe, então retorna erro
        if(userExists) {
