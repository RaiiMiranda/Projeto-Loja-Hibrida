...
export const orderModel = {
    ...
    // Criando um pedido
    async create(order) {
        const { created_at, channel, status, total_value, user_id, address_id } = order;
        
        const query = `
            INSERT INTO order_client (created_at, channel, status, total_value, user_id, address_id)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *
        `;

        const values = [created_at, channel, status, total_value, user_id, address_id];
